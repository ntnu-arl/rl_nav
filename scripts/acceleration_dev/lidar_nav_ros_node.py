...
class LidarNavigationNode:
    def __init__(self, device="cuda:0"):
        ...
        self.pointcloud_sub = rospy.Subscriber("/rslidar_points", PointCloud2, self.pointcloud_callback, queue_size=1)
    ...
    def pointcloud_callback(self, msg):
        ...
        # print the organized pointcloud such taht I can use it in an ipynb for downsampling manually
        print("Received pointcloud with width:", msg.width, "height:", msg.height)
        # Bin edges
        points3d = msg.data
        ...
        # print min and max indices
        print("Azimuth idx min:", azimuth_idx.min().item(), "max:", azimuth_idx.max().item())
        print("Elevation idx min:", elevation_idx.min().item(), "max:", elevation_idx.max().item())
        ...
        input_tensor = torch.full((azimuth_bins * elevation_bins,), 20.0)
        ...
        self.lidar_tensor = self.lidar_tensor / 10.0

        self.lidar_tensor[:] = 1.0
        ...
        print("input state tensor:", self.state_obs_cpu[:17].cpu().numpy())
        print("Action from pointcloud:", action)
